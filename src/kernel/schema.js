...
function _sanitize(v, s) {
  ...
  switch (s.type) {
    ...
    case "array": {
      ...
      // HARDENING: TypedArrays are converted to plain arrays and length-limited.
      const sliced = Array.from(arr.slice(0, effectiveLen));
      const out = sliced.map(x => _sanitize(x, itemSchema));
      return out;
    }
    case "object": {
      const shape = s.shape;
